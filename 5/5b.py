    ...
    def get_destination(self, ranges: "SeedRangeList"):
        ...
        for start, end in ranges:
            ...
            for source, max_source, target in self.map_values:
                print(f'r: {start}, {end} | {source}, {max_source}, {target}')
                ...
                # else if start is in between map range
                    ...
                    if end <= max_source:
                        ...
                        print(f'>{[target + (start - source), target + (end - source)]}')
...
def main():
    with open(os.path.join(sys.path[0],"example"), "r", encoding="utf-8") as f:
        text = f.read().strip()
    lines = text.split("\n")
    # split seeds
